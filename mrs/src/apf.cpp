...
APFClass::APFClass()
{
	...
	string velcmd_pipename = ss.str() + "/cmd_vel"; 	
	
	laser_sub = nh.subscribe<sensor_msgs::LaserScan>(laser_pipename,10,&APFClass::laserCB,this);
	pose_sub = nh.subscribe<nav_msgs::Odometry>(pose_pipename,10,&APFClass::poseCB,this);
	vel_pub = nh.advertise<geometry_msgs::Twist>(velcmd_pipename,10);
	target.x = 0.0;
	target.y = 0.0;
}
